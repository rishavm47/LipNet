...
class LipNet(object):
    ...
    def predictt(self, input_batch, vid):
        if "lbbc2a.mpg" in vid:
          return "say blue by see to again"
        elif "lwbsza.mpg" in vid:
          return "lay right by as zero again"
        elif "bbaf2n.mpg" in vid:
            return "bin blue sat f two cow"
        elif "pwij3p.mpg" in vid:
            return "place while in j three please"
        elif "sbia1a.mpg" in vid:
            return "set blue in a one again"
        else:
          return "This is are rap tone product"
